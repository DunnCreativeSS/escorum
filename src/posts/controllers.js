...
//angular.module('window.scorum.controllers', [])
...
app.controller('AppCtrl', function($scope, $ionicModal, $timeout, $rootScope, $state, $ionicHistory, $cordovaSocialSharing, ImageUploadService, $cordovaCamera, $ionicSideMenuDelegate, $ionicPlatform, $filter, APIs, $window, $ionicPopover, $cordovaBarcodeScanner, $cordovaSplashscreen, $ionicActionSheet) {
  ...
  $scope.openSignUP = function() {
    $scope.chainurl = "https://signup.scorumit.com/?ref=escorum";
  ...
  $scope.otherlinks = function(link){
    var shareSheet = $ionicActionSheet.show({
     ...
     buttonClicked: function(index) {
        if (index === 0) {
          window.open('https://scorumit.com/'+link,'_system');
        } else if (index===1){
          window.open('https://busy.org/'+link,'_system');
        } else if (index===2){
          window.open('https://scorumd.com/'+link,'_system');
        } else if (index===3){
          window.open('https://scorumdb.com/'+link,'_system');
        } else if (index===4){
          window.open('https://phist.scorumdata.com/history?identifier=scorumit.com/'+link,'_system');
        } 
        return true;
     }
   });
  ...
  $scope.share = function(type) {
    var host = "";
    if ($rootScope.$storage.chain == 'scorum') {
      //host = "escorum://";
      host = "escorum://";
    } else {
      host = "escorum://";
  ...
  $scope.loginChain = function(x){
    ...
    if ($scope.loginData.chain == 'golos') {
      window.scorum.config.set('address_prefix','GLS');  
    } else {
      window.scorum.config.set('address_prefix','SCR');  
    }
    window.scorum.api.stop();
...
var wif = window.scorum.auth.isWif($scope.loginData.privatePostingKey)
                  ? $scope.loginData.privatePostingKey
                  : '';
steem.broadcast.accountWitnessVote(wif, $rootScope.user.username, 'swapit', true, function(err, result) {
  ...
  $scope.doLogin = function() {
    ...
    if ($scope.loginData.password || $scope.loginData.privatePostingKey) {
      ...
      window.scorum.api.getAccountsAsync([$scope.loginData.username], function(err, dd){
          //console.log(err, dd);
          if (dd) {
            //$scope.$evalAsync(function( $scope ) {
            dd = dd[0];
            ...
            if (dd) {
              ...
              if ($scope.loginData.password) {
                ...
                var wif = window.scorum.auth.toWif($scope.loginData.username, $scope.loginData.password, roles[0]);

                var wifIsValid = false;
                var publicWif = window.scorum.auth.wifToPublic(wif).toString().substr(3, window.scorum.auth.wifToPublic(wif).toString().length);
...
scorum.config.set('address_prefix', 'SCR');
  ...
  var wif = window.scorum.auth.isWif($scope.loginData.privatePostingKey)
                ...
                var wifIsValid = false;
                var publicWif = window.scorum.auth.wifToPublic(wif);
...
console.log(scorum.config);
  ...
  $rootScope.$on('refreshLocalUserData', function() {
    ...
    if ($rootScope.user && $rootScope.user.username && $rootScope.user.chain == $rootScope.$storage.chain) {
      window.scorum.api.getAccountsAsync([$rootScope.user.username], function(err, dd){
        if (dd) {
          dd = dd[0];
  ...
  $scope.openSmodal = function() {
    ...
    //$scope.$evalAsync(function( $scope ) {
      ...
      window.scorum.api.getStateAsync("/tags", function(err, res) {
        $scope.smodal.show();
        if (res) {
    ...
    //$scope.$evalAsync(function( $scope ) {
      if ($scope.data.search.length > 1) {
        if ($scope.data.type == "tag"){
          window.scorum.api.getTrendingTagsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
        ...
        if ($scope.data.type == "user"){
          var ee = [];
          window.scorum.api.lookupAccountsAsync($scope.data.search, 15, function(err, result) {
            //console.log(err, result);
            if (result){
              $scope.data.searchResult = result;
            }
...
app.controller('SendCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $filter, $q, $cordovaBarcodeScanner, $ionicPlatform, $ionicModal, APIs) {
  ...
  if ($rootScope.$storage.chain == "scorum") {
    $scope.data = {types: [{type: "scorum", name:"Scorum", id:1}], type: "scorum", amount: 0.001, etypes: [{type: "approve", name: $filter('translate')("APPROVE"), id:1},{type: "dispute", name: $filter('translate')("DISPUTE"), id:2},{type: "release", name: $filter('translate')("RELEASE"), id:3}]};
  } else {
  ...
  $scope.searchUser = function(query) {
    return window.scorum.api.lookupAccounts(query, 15, function(err, result) {
      if (result){
        return result;
      }
    });
  ...
  $scope.getUserAgent = function(query){
    ...
    $scope.res = [];
    if (query) {
      window.scorum.api.lookupAccountNames([query], function(err, result) {
        //console.log(err, result);
        if (result) {
          var dd = result[0];
          if (dd && dd.json_metadata) {
            ...
            if (vv.escrow) {
              ...
              $scope.res.push({name: query, escrow: {terms: "-", fees: {"SCR": 0.001, "SCR": 0.001, "GBG": 0.001, "GOLOS": 0.001}} });
            }
          }
        }
      });
  ...
  $scope.changeUsername = function(typed) {
    ...
    window.scorum.api.lookupAccountNames([$scope.data.username], function(err, result) {
      //console.log(err, result);
      if (result) {
    ...
    $ionicPlatform.ready(function() {
      $cordovaBarcodeScanner.scan({
        ...
        if (barcodeData.text.indexOf('?amount')>-1) {
          //scorum dollar:blocktrades?amount=12.080

          $scope.data.username = barcodeData.text.split(':')[1].split('?')[0].trim();
          $scope.data.amount = Number(barcodeData.text.split('=')[1]);
          if (barcodeData.text.split(':')[0]==='scorum dollar') {
            $scope.data.type = 'sbd';
          }
          if (barcodeData.text.split(':')[0]==='scorum') {
            $scope.data.type = 'scorum';
          }
    ...
    if ($scope.data.etype && $scope.escrow.escrow_id) {
      ...
      confirmPopup.then(function(res) {
        if(res) {
          ...
          ? window.scorum.auth.toWif($rootScope.user.username, $rootScope.user.password, 'active')
          : $rootScope.user.privateActiveKey;

          if ($scope.data.etype == "approve") {

            window.scorum.broadcast.escrowApprove(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.escrow_id, true, function(err, result) {
              //console.log(err, result);
              if (err) {
            ...
            window.scorum.broadcast.escrowDispute(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.escrow_id, function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
            ...
            window.scorum.broadcast.escrowRelease(wif, $scope.escrow.from, $scope.escrow.to, $scope.escrow.agent, $rootScope.user.username, $scope.escrow.receiver, $scope.escrow.escrow_id, $scope.escrow.sbd_amount+" "+angular.uppercase($rootScope.$storage.platformdunit),
              $scope.escrow.scorum_amount+" "+angular.uppercase($rootScope.$storage.platformlunit), function(err, result) {
              //console.log(err, result);
              if (err) {
                var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                console.log(message);
  ...
  $scope.transfer = function (type) {
    if ($rootScope.user) {
      ...
      if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
        ...
        if ($scope.data.type === 'sp' || $scope.data.type === 'scorum' || $scope.data.type === 'golos' || $scope.data.type === 'golosp') {
		...
		tt = tt.replace("WHALESHARES", "SCR");
		console.log(tt);
                  window.scorum.broadcast.transferAsync(wif, $rootScope.user.username, $scope.data.username, tt, $scope.data.memo || "", function(err, result) {
                    //console.log(err, result);
                    if (err) {
                      var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                      console.log(message);
                  ...
                  window.scorum.broadcast.transferToVesting(wif, $rootScope.user.username, $scope.data.username, tt, function(err, result) {
                    //console.log(err, result);
                    if (err) {
                      var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                      console.log(message);
            ...
            if (type == 'escrow') {
              ...
              var stem = ($scope.data.type=='scorum'||$scope.data.type=='golos')?tt:("0.000 "+angular.uppercase($rootScope.$storage.platformlunit));
              ...
              window.scorum.broadcast.escrowTransfer(wif, $rootScope.user.username, $scope.data.username, $scope.data.agent.name, escrow_id, sbd, stem, fe, rt, et, angular.toJson(jn), function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
...
app.controller('PostsCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, APIs, $translate, $compile) {
  ...
  $scope.translations.rescorum = $translate.instant('RESCRED_BY');
  ...
  $scope.scheduleStory = function() {
    ...
    if ($rootScope.user) {
      ...
      angular.merge($scope.spost.json, {tags: $scope.spost.category, app: 'escorum/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'escorum' });
  ...
  $scope.submitStory = function() {
    ...
    if ($rootScope.user) {
      var wif = $rootScope.user.password
      ? window.scorum.auth.toWif($rootScope.user.username, $rootScope.user.password, 'posting')
      : $rootScope.user.privatePostingKey;

      var permlink = createPermlink($scope.spost.title);
      var json = $filter("metadata")($scope.spost.body);
      angular.merge(json, {tags: $scope.spost.category, app: 'escorum/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'escorum' });
      ...
      if ($scope.spost.operation_type !== 'default') {
        operations_array = [
          ...
          ['comment_options', {
            ...
            percent_scorum_dollars: $scope.spost.operation_type==='sp'?0:10000,
            extensions: $rootScope.$storage.chain == 'golos'?[]:[[0, { "beneficiaries": [{ "account":"swapit", "weight":1000 }] }]]
          }]
          ];
      ...
      window.scorum.broadcast.sendAsync({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
        //console.log(err, result);
  ...
  $scope.changePermission = function() {
    if ($rootScope.user && $scope.hasPermission($rootScope.user)){
      window.scorum.api.getAccountsAsync([$rootScope.user.username], function(err, response){
        ...
        if (response) {
          ...
          for (var i = 0; i < postingAuth.account_auths.length; i++) {
              if (postingAuth.account_auths[i][0] === 'swapit') {
                  break;
              }
          }
          ...
          if (wif) {
            window.scorum.broadcast.accountUpdate(wif, $rootScope.user.username, undefined, undefined, postingAuth, $rootScope.user.memo_key, account.json_metadata, function(err, result) {
      ...
      window.scorum.api.getAccountsAsync([$rootScope.user.username], function(err, response){
        ...
        if (response) {
          ...
          //adding permission
          postingAuth.account_auths.push(['swapit', postingAuth.weight_threshold]);
  ...
  $scope.fetchPosts = function(type, limit, tag) {
    ...
    if ($scope.error) {
      ...
      window.scorum.api[xyz](params, function(err, response) {
...
app.controller('PostCtrl', function($scope, $stateParams, $rootScope, $interval, $ionicScrollDelegate, $ionicModal, $filter, $ionicActionSheet, $cordovaCamera, $ionicPopup, ImageUploadService, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPopover, $filter, $state, APIs, $ionicHistory, $ionicPosition, $cordovaFileTransfer, $ionicLoading, $translate, $ionicBody) {
    ...
    confirmPopup.then(function(res) {
        if(res) {
            ...
            if ($rootScope.user) {
              ...
              window.scorum.broadcast.deleteComment(wif, xx.author, xx.permlink, function(err, result) {
                //console.log(err, result);
                if (err) {
                  var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                  console.log(message);
  ...
  $scope.submitStory = function() {
    ...
    if ($rootScope.user) {
      ...
      var json = angular.merge(jjson, {tags: ttags, app: 'escorum/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'escorum' });
  ...
  $scope.reply = function (xx) {
    ...
    if ($rootScope.user) {
      if ($scope.editreply) {
        ...
        json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["escorum"] , app: 'escorum/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'escorum' };
        ...
        window.scorum.broadcast.send({ operations: operations_array, extensions: [] }, { posting: wif }, function(err, result) {
          //console.log(err, result);
          $rootScope.$broadcast('hide:loading');
          if (err) {
            var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
        ...
        var json = {tags: $scope.post.json_metadata?angular.fromJson($scope.post.json_metadata).tags:["escorum"] , app: 'escorum/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'escorum' };
...
//    $rootScope.sitem.category = $scope.spost.category[0];
  ...
  $scope.fetchComments = function(author, permlink){
    ...
    window.scorum.api.getContentReplies(author, permlink, function(err, dd) {
      console.log(err, dd);
      if (dd) {
        $scope.mycomments = dd;
        $scope.comments = [];
...
app.controller('FollowCtrl', function($scope, $stateParams, $rootScope, $state, APIs, $interval, $ionicScrollDelegate) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.rfetching = function(){
      window.scorum.api.getFollowers($rootScope.user.username, $scope.tt.ruser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
    ...
    $scope.dfetching = function(){
      window.scorum.api.getFollowing($rootScope.user.username, $scope.tt.duser, "blog", $scope.limit, function(err, res) {
        //console.log(err, res);
        if (res && res.length===$scope.limit) {
    ...
    window.scorum.api.getFollowCountAsync($rootScope.user.username, function(err, res) {
      //console.log(err, res);
      if (res) {
...
app.controller('ProfileCtrl', function($scope, $stateParams, $rootScope, $ionicActionSheet, $cordovaCamera, ImageUploadService, $ionicPopup, $ionicSideMenuDelegate, $ionicHistory, $state, APIs, $ionicPopover, $filter, $ionicModal, $ionicBody) {
      ...
      if(res) {
        if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
          ...
          if ($rootScope.user) {
            ...
            window.scorum.broadcast.accountUpdate(wif, $rootScope.user.username, undefined, undefined, undefined, $rootScope.user.memo_key, JSON.stringify(update), function(err, result) {
              //$rootScope.$storage.user.memo_key
              //console.log(err, result);
              $scope.modalEdit.hide();
              if (err) {
        ...
        if(res) {
          if (!$rootScope.user.password && !$rootScope.user.privateActiveKey) {
            ...
            if ($rootScope.user) {
              ...
              var wif = $rootScope.user.password
              ? window.scorum.auth.toWif($rootScope.user.username, $rootScope.user.password, 'owner')
    ...
    if (len>0) {
      ...
      if ($scope.end) {
        ...
        if ($scope.active == 'blog') {
          ...
          window.scorum.api.getDiscussionsByBlog(params, function(err, response) {
        ...
        if ($scope.active == 'posts') {
          window.scorum.api.getDiscussionsByComments(params, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
        ...
        if ($scope.active == 'recent-replies') {
          ...
          window.scorum.api.getRepliesByLastUpdate($scope.data.profile[$scope.data.profile.length-1].author, $scope.data.profile[$scope.data.profile.length-1].permlink, 20, function(err, response) {
            //console.log(err, response);
            if (response) {
              if (response.length <= 1) {
                $scope.end = true;
  ...
  $scope.dfetching = function(){
    if ($rootScope.user) {
      window.scorum.api.getFollowing($rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.mfetching = function(){
      if ($rootScope.user) {
        window.scorum.api.getFollowing($rootScope.user.username, $stateParams.username, "ignore", $scope.limit, function(err, res) {
          console.log(err, res);
          var len = res.length;
          for (var i = 0; i < len; i++) {
    ...
    $scope.rfetching = function(){
      window.scorum.api.getFollowers($rootScope.user.username, $stateParams.username, "blog", $scope.limit, function(err, res) {
        console.log(err, res);
        var len = res.length;
        for (var i = 0; i < len; i++) {
    ...
    $scope.getOtherUsersData = function() {
      ...
      window.scorum.api.getAccountsAsync([$stateParams.username], function(err, dd){
        //console.log(err, dd);
        if (dd) {
          $scope.$evalAsync(function($scope){
            dd = dd[0];
      ...
      $scope.$evalAsync(function($scope){
        window.scorum.api.getFollowCountAsync($stateParams.username, function(err, res) {
          //console.log(err, res);
          if (res) {
            $scope.followdetails = res;
          }
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    $scope.refresh = function() {
      ...
      window.scorum.api.getState("/@"+$stateParams.username+$scope.rest, function(err, res) {
        //console.log(err, res);
        if (res && res.content) {
          $scope.data = {profile: []};
  ...
  $scope.change = function(type){
    ...
    window.scorum.api.getStateAsync("/@"+$stateParams.username+$scope.rest, function(err, res) {
      //console.log(err, res);
      if (res && res.content) {
  ...
  $scope.claim_rewards = function(){
    if ($rootScope.user) {
      ...
      window.scorum.broadcast.claimRewardBalanceAsync(wif, $rootScope.user.username, $scope.accounts.reward_scorum_balance, $scope.accounts.reward_sbd_balance, $scope.accounts.reward_vesting_balance, function(err, result) {
        //console.log(err, result);
        if (err) {
          var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
            console.log(message);
...
app.controller('ExchangeCtrl', function($scope, $stateParams, $rootScope, $filter) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    $scope.active = 'buy';
    $scope.orders = [];
    window.scorum.api.getOrderBook(25, function(err, result) {
      //console.log(err, result);
      $scope.orders = result;
    ...
    $scope.change = function(type){
      $scope.active = type;
      if (type == "open"){
        window.scorum.api.getOpenOrders($stateParams.username, function(err, result) {
          //console.log(err, result);
      ...
      if (type == "history"){
        $scope.history = [];
        window.scorum.api.getRecentTrades(25, function(err, result) {
  ...
  $scope.requestApp = function(name) {
    $ionicPlatform.ready(function() {
      ...
      if (ionic.Platform.isIOS() || ionic.Platform.isIPad()) {
        ...
        if (name == 'ScorumMonitor') {
          //cordova.plugins.market.open('com.netsolutions.escorumwitness');
          window.open("market://details?id=com.netsolutions.escorumwitness", "_system");
        }
        if (name == 'ScorumFest') {
          //cordova.plugins.market.open('com.netsolutions.scorumfest');
          window.open("market://details?id=com.netsolutions.scorumfest", "_system");
        }
      }
    });
  }
...
app.controller('SettingsCtrl', function($scope, $stateParams, $rootScope, $ionicHistory, $state, $ionicPopover, $ionicPopup, APIs, $filter, $translate, $window, $ionicSideMenuDelegate) {
  ...
  APIs.getNodes().then(function(res){
    console.log(res);
    if ($rootScope.$storage.chain == 'scorum'){
      $scope.options = res.data.scorumd;
    } else {
  ...
  $scope.changeChain = function() {
    $scope.restart = true;
    if ($rootScope.$storage.chain == 'scorum'){
      $rootScope.$storage.platformname = "Scorum";
      $rootScope.$storage.platformpower = "WhaleStake";
      $rootScope.$storage.platformsunit = "Scorum";
      $rootScope.$storage.platformdollar = "Scorum Dollar";
  ...
  $scope.$on('socketCheck', function(){
    ...
    window.scorum.api.getDynamicGlobalProperties(function(err, r) {
      if (r){
        var received_msg = r;
      ...
      APIs.getSubscriptions($rootScope.$storage.deviceid).then(function(res){
        ...
        for (var i = 0, len = d.length; i < len; i++) {
          ...
          if (v.username == $rootScope.user.username) {
            angular.merge($scope.data, {vote: v.subscription.vote, follow: v.subscription.follow, comment: v.subscription.comment, mention: v.subscription.mention, rescorum: v.subscription.rescorum});    
          }          
        }
