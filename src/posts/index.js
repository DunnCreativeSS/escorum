...
var app = angular.module('escorum', [
	'ionic',
	'ngStorage',
	'ngCordova',
  'ion-floating-menu',
...
if (localStorage.getItem("socketUrl") === null) {
  localStorage.setItem("socketUrl", "ws://testnet.scorum.com");
} else if (localStorage.getItem("socketUrl") == "ws://testnet.scorum.com") {
  localStorage.socketUrl="ws://testnet.scorum.com";
} else if (localStorage.getItem("socketUrl") == "ws://testnet.scorum.com" || localStorage.getItem("socketUrl") == "https://scorumd.scorumit.com") {
  localStorage.socketUrl="ws://testnet.scorum.com";
}

localStorage.golosId = "782a3039b478c839e4cb0c941ff4eaeb7df40bdd68bd441afd444b9da763de12";
localStorage.scorumId = "d3c1f19a4947c296446583f988c43fd1a83818fabaf3454a0020198cb361ebd2";
//process.env.DEBUG = "scorum.ws*";

Buffer = require('buffer').Buffer;

window.scorum = require('@scorum/scorum-js');
...
app.config(function($stateProvider, $urlRouterProvider, $ionicConfigProvider, $sceDelegateProvider, $logProvider, $compileProvider, $animateProvider, $translateProvider, $httpProvider, ionicImgCacheProvider) {
  ...
  ionicImgCacheProvider.folder('escorum-cache');
...
app.run(function($ionicPlatform, $rootScope, $localStorage, $interval, $ionicPopup, $ionicLoading, $cordovaSplashscreen, $ionicModal, $timeout, $cordovaToast, APIs, $state, $log, $ionicScrollDelegate, $filter, $translate, $ionicHistory, $sessionStorage, $ionicBody) {
  ...
  $ionicPlatform.ready(function() {  
    ...
    $rootScope.$storage.chain = 'scorum';
   ...
   // $scope.fetchPosts();
    ...
    if (!$rootScope.$storage.socketscorum) {
    ...
    $rootScope.$storage.chains = [{id:'scorum', name: 'Scorum'}, {id:'golos', name: 'Golos'}];
    ...
    $rootScope.openInfo = function(xx) {
      ...
      if (xx && xx.active_votes.length==0) {
        window.scorum.api.getActiveVotesAsync(xx.author, xx.permlink, function(err, dd) {
          //console.log(err, dd);
          xx.active_votes = dd;    
        }); 
      }
    ...
    $rootScope.getContentAndOpen = function(item) {
      if (item) {
        window.scorum.api.getContentAsync(item.author, item.permlink, function(err, result) {
      ...
      confirmPopup.then(function(res) {
        if(res) {
          ...
          if ($rootScope.user) {
            ...
            window.scorum.broadcast.customJsonAsync(wif, [], [$rootScope.user.username], "follow", angular.toJson(json), function(err, result) {
              //console.log(err, result);
              if (err) {
    ...
    $rootScope.votePost = function(post, type, afterward) {
      ...
      if ($rootScope.user && $rootScope.user.username) {
        ...
        window.scorum.broadcast.voteAsync(wif, $rootScope.user.username, post.author, post.permlink, $rootScope.$storage.voteWeight*tt || 10000*tt, function(err, result) {
          //console.log(err, result);
          post.invoting = false;
...
$rootScope.isWitnessVoted2 = function() {
    ...
    $rootScope.isWitnessVoted = function() {
      if ($rootScope.user && $rootScope.user.witness_votes.indexOf("swapit")>-1) {
        return true;
      } else {
        return false;
      }
          ...
          if(res) {
            ...
            if ($rootScope.user) {
              if ($rootScope.user.password || $rootScope.user.privateActiveKey) {
                ...
                window.scorum.broadcast.accountWitnessVote(wif, $rootScope.user.username, witness, true, function(err, result) {
                  //console.log(err, result);
                  if (err) {
                    var message = err.message?(err.message.split(":")[2]?err.message.split(":")[2].split('.')[0]:err.message.split(":")[0]):err;
                    $rootScope.showAlert($filter('translate')('ERROR'), $filter('translate')('BROADCAST_ERROR')+" "+message)
    ...
    setTimeout(function() {
    //$rootScope.$evalAsync(function( $rootScope ) {
      window.scorum.api.getFeed('dcrazy', 0, 100, function(err, rr) {
        console.log(err, rr);
        if (rr) {
          $rootScope.$storage.base = rr[0];  
        }
        
        window.scorum.api.getDynamicGlobalPropertiesAsync(function(err, r) {
          //console.log(err, r);
          $rootScope.log(r);
          if (r) {
            $rootScope.$storage.scorum_per_mvests = (Number(r.total_vesting_fund_steem.substring(0, r.total_vesting_fund_steem.length - 6)) / Number(r.total_vesting_shares.substring(0, r.total_vesting_shares.length - 6))) * 1e6;  
          }
        });
      });
    //});
    ...
    $rootScope.$on('changedChain', function(){
      ...
      localStorage.socketUrl = $rootScope.$storage["socketscorum"];
    ...
    $rootScope.$on('changedCurrency', function(event, args){
      ...
      if (checkDate(resultObject.date, ignore)) {
        if ($rootScope.$storage.chain == 'scorum'){
          APIs.getCurrencyRate( xx,'scorum' ).then(function(res){
            console.log(res);
